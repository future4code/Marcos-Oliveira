...
class App extends React.Component {
  ...
  confirmExcluirPlaylist = idPlaylist => {
    const exclusao = window.confirm("Deseja excluir a Playlist?")
    if (exclusao) {
      this.deletaPlaylist(idPlaylist)
    }
  }
  ...
  playlistEscolhida = (event) => {
    ...
    idPlaylist = playlistFiltrada.map(element => element.id).toString()
  ...
  addMusicaPlaylist = () => {
    ...
    Axios.post(`https://us-central1-labenu-apis.cloudfunctions.net/labefy/playlists/${idPlaylist}/tracks`,body,
      {
        headers: {
          Authorization: "marcos-oliveira-mello"
        }
